...
<sch:schema xmlns:sch="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt3"
    ...
    <!-- ================================================================ -->
    <sch:pattern>
        <sch:rule context="morning/slot[1]/@time">
            <sch:p>First morning slot should start at 11:00 a.m. on Monday and 9:00 on other
                days</sch:p>
            <sch:assert test="
                    if (ancestor::day/@d eq 'Monday, June 15') then
                        . eq '11:00:00'
                    else
                        . eq '09:00:00'">Monday starts at 11:00 a.m., other days at 9:00</sch:assert>
        </sch:rule>
        <sch:rule context="afternoon/slot[1]/@time">
            <sch:p>first afternoon slot should start at 1:30, except that there is no second slot on
                Friday</sch:p>
            <sch:assert test="
                    if (ancestor::day/@d ne 'Friday, June 19') then
                        . eq '13:30:00'
                    else
                        0">There is no afternoon session on Friday</sch:assert>
            <sch:assert test="ancestor::day/@d ne 'Friday, June 19'">There is no afternoon session on Friday</sch:assert>
        </sch:rule>
        ...
        <sch:rule context="morning">
            ...
            <sch:let name="duration" value="sum(slot/act/@time/xs:dayTimeDuration(.))"/>
            <sch:assert test="
        ...
        <sch:rule context="afternoon">
            <sch:p>Total duration of all afternoon slots is 2:30.</sch:p>
            <sch:let name="duration" value="sum(slot/act/@time/xs:dayTimeDuration(.))"/>
    ...
    <sch:pattern>
        <sch:rule context="morning[preceding::morning] | afternoon">
            <sch:assert test="count(slot/act[desc eq 'Break']) eq 1">Every slot except Monday morning must have exactly one break</sch:assert>
        </sch:rule>
    </sch:pattern>
</sch:schema>
